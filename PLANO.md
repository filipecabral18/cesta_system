...
- [ ] Criar conta gratuita no Supabase (supabase.com) e criar um projeto chamado `cesta`.
- [ ] Anotar a `Project URL` e a `anon public key` (Settings → API).
- [ ] Ter Node.js LTS instalado.
- [ ] Instalar o Claude Code e abrir a pasta do projeto nele.
...
- [ ] Depois: criar o arquivo `.env` local com suas chaves reais (não commitar).
...
- [ ] Depois: aplicar a migration no SQL Editor do Supabase e habilitar Auth por e-mail.
